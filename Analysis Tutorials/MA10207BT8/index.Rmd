...
```{r,sinapx,fig.show='hold',fig.cap="Taylor approximations to $\\sin(x)$ at $x=0$ of degree 1 (red), 3 (orange), 5 (yellow), 7 (green), 9 (blue), 11 (purple) and 13 (pink). Taken from [Wikipedia](By IkamusumeFan - Own work, CC BY-SA 3.0, https://commons.wikimedia.org/w/index.php?curid=27865201).", fig.alt="A diagram illustrating Taylor's Theorem", out.width = "50%", echo=F}
show_fig("sinapx")
```
...
```{r,bob,fig.show='hold',fig.cap="A pendulum consisting of a string of length $\\ell$ with a mass $m$ attached to one end. The pendulum is allowed to perform planar oscillations under the influence of gravity $g$, which causes the angle to the vertical, $\\theta$, to change with time.", fig.alt="The setup for a simple pendulum.", echo=F}
show_fig("bob")
```

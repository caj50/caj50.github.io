...
Here is the material to accompany the 2nd Analysis Tutorial in Week 2. Alternative formats can be downloaded by clicking the download icon at the top of the page. As usual, send comments and corrections to \href{mailto:caj50@bath.ac.uk}{Christian Jones (caj50)}. To return to the homepage, click \href{http://caj50.github.io/tutoring.html}{here}.
...
Finally, we are ready to state the required \(15^{th}\) axiom! As the title suggests, this is known as the \emph{Completeness Axiom}.

\BeginKnitrBlock{Completeness Axiom*}
{}Every non-empty set \(S\) in \(\mathbb{R}\) that is bounded above has a supremum\footnote{In the lecture notes, it also states that `Every non-empty set of real numbers that is bounded below has an infimum.' But you can deduce this from the supremum result by considering the set \(-S:=\lbrace -x \lvert x \in S\rbrace.\)}.

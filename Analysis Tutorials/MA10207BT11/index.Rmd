...
```{r,additivity,fig.show='hold',fig.cap="The integral being additive means that to integrate a function $f$ on a domain $[a,b]$, we can just sum up the integrals of $f$ on some smaller domains. This is especially useful for functions defined piecewise.", fig.alt="A diagram illustrating the additivity of an integral.", fig.align="center", echo=F}
show_fig("Additivity")
```
...
```{theorem, label="thm3", name = "Fundamental Theorem of Calculus I"}
...
```{theorem, label="thm4", name = "Fundamental Theorem of Calculus II"}
...
```{theorem, label="thm5", name = "Integration By Parts"}
...
```{theorem, label="thm6", name = "Integration By Substitution"}

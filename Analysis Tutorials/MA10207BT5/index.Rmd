...
```{example}
Consider the exponential function $\exp:\mathbb{R} \to \mathbb{R}$ defined by $$f(x)  = \sum_{n = 0}^{\infty} \frac{x^n}{n!} = \mathrm{e}^x.$$ Firstly, note that $\mathbb{R}$ is a non-empty interval. Now, using some results from Semester 1, we know that

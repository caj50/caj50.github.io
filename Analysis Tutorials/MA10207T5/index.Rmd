...
```{definition, label = "def1", name = "Monotonic Sequences"}
...
```{theorem, label = "thm2", name = "Sandwich Theorem"}
...
```{theorem, label = "thm3", name = "Bitten Sandwich Theorem"}
Suppose that $(a_n)_{n\in\mathbb{N}}\, , \, (b_n)_{n\in\mathbb{N}}\, , \, (c_n)_{n\in\mathbb{N}}$ are real sequences. If $\exists N \in \mathbb{N}$ such that $a_n \leq b_n \leq c_n \; \forall n \geq N$, and $\exists L \in \mathbb{R}$ such that
\begin{align*}
    \lim_{n \to \infty} a_n = L = \lim_{n \to \infty} c_n,
\end{align*}
then $\lim_{n \to \infty}b_n = L$.
...
```{definition, label = "def2", name = "Divergence to Infinity"}
...
```{theorem, label = "thm4", name = "Algebra of Infinite Limits"}

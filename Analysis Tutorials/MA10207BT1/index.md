...
# Introduction {-}
Here is the material to accompany the 1st Analysis Tutorial on the 6th February. Alternative formats can be downloaded by clicking the download icon at the top of the page. As usual, send comments and corrections to [Christian Jones (caj50)](mailto:caj50@bath.ac.uk). To return to the homepage, click [here](http://caj50.github.io/tutoring.html).

# Lecture Recap
This is somewhat a disingenuous title, as there haven't been any lectures for this unit yet! Instead, we are going to recap some of the material from last semester, which will come in handy for the content you'll see over the next eleven weeks. The material here is possibly quite terse --- more details can be found in the Analysis 1A files from Semester 1.
...
As you saw last semester, this notation can get pretty annoying, so instead we use the notation $(a_n)_{n\in\mathbb{N}}$. If it's clear from the context what set we're indexing over, we can even just write $(a_n)_n$.

Since the natural numbers forms a countably infinite set, a sequence gives us a countably infinite list of real numbers. Sometimes it's interesting to look at the 'long-term' behaviour of this list, which gives us the idea of convergence:
...
Loosely speaking, this says that no matter how close you want the sequence to get to $L$, you will always be able to find some point in the sequence after which all points in the sequence will be as close to $L$ as you wanted.
...
The first test is the *sandwich/pinching/squeeze* theorem:
...
In words, this says that if you can 'trap' the sequence under consideration between two other sequences which converge to a common limit, then all three sequences involved will converge to the same limit.
...
Note that this test won't work if the terms of the sequence $(a_n)_n$ are given by ratios of polynomials (can you see/prove why?)

These two tests are good at proving convergence, but is there a quick way of proving non-convergence? As you've seen before, we can do this using subsequences:
...
The mathematics in this section is nothing new either --- it's really just sequences in disguise. Given a sequence $(a_n)_n$, one thing we can do is add up all the terms and see what happens. This leads on to the idea of an (infinite) series:
...
So when does this series converge? In other words, when is this object a real number? To answer these questions, we have the following definition:
...
Again, this test fails if the series terms are formed of ratios of polynomials.
...
## Sets and Bounds
The final topic we are going to re-cover here involves sets of real numbers, and deciding on whether we can 'trap' these sets between an upper bound $M$ --- so that no member of the set lies above $M$ --- and a lower bound $m$ --- so that no member of the set lies below $m$. This gives rise to the defintion of a bounded set, as is seen below:
...
Finally, we can alternatively characterise the supremum/infimum of a set as follows:

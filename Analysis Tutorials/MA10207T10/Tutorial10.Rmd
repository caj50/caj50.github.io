...
```{definition, label="def1",name = "Intervals"}
...
```{example, label = "ex1"}
Prove that the function $f: \mathbb{R} \to \mathbb{R}$ given by $$f(x) = x^{27} - 4x^6 + \frac{3}{x^2 +1}$$ is sequentially continuous at any $x_0 \in \mathbb{R}$.
```

```{solution, label="sol1"}
First take *any* sequence $(x_n)_{n\in\mathbb{N}}$ in $\mathbb{R}$ such that $x_n \to x_0$ as $n \to \infty$. Then by the Algebra of Limits
